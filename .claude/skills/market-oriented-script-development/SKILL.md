...
| 输入 | 路径 | 用途 |
|------|------|------|
| 创意设定文件 | `project_data/series/creative-bible-vX.yaml` | 剧本展开的基准输入 |
| 创意阶段交接摘要 | `project_data/series/script-handoff-vX.yaml` | 了解交接要求 |
| 历史剧本版本 | `project_data/episodes/epXX/script-vX.yaml` | 改稿时参考 |
| 用户本轮反馈 | 聊天上下文 | 改稿方向 |
...
| 产物 | 输出路径 | 触发条件 |
|------|---------|---------|
| 故事梗概 | `project_data/series/synopsis-vX.yaml` | 首次进入剧本阶段 |
| 分集规划 | `project_data/series/episode-plan-vX.yaml` | 梗概完成后 |
| 单集剧本 | `project_data/episodes/epXX/script-vX.yaml` | 分集规划完成后 |
...
读取 `project_data/series/creative-brief-vX.yaml`（status 必须为 approved）。
如果存在交接摘要，一并读取。

确认以下信息：
- title、logline、genre、target_audience
- characters（每个角色的 role、description、arc、visual_keywords）
- world（setting、era、visual_style、color_palette）
- episode_plan（total_episodes、episode_duration、narrative_structure）
...
将梗概写入 `project_data/series/synopsis-v1.0.yaml`，status = draft。
...
将分集规划写入 `project_data/series/episode-plan-v1.0.yaml`，status = draft。
...
将单集剧本写入 `project_data/episodes/epXX/script-v1.0.yaml`，status = draft。

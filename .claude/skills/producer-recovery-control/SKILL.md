...
| 当前阶段产物 | `project_data/episodes/epXX/` | 定位失败发生点 |
| 审核结果 | `project_data/episodes/epXX/reviews/` | 确认是哪种审核失败 |
...
| 失败类型 | 识别方式 |
|---------|---------|
| 业务审核失败 | 审核结论 verdict = rejected，reviewer = business-review-agent |
| 合规审核失败 | 审核结论 verdict = rejected，reviewer = compliance-review-agent |
